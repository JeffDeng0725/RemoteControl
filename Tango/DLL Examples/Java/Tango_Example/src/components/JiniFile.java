...
public class JiniFile extends ArrayList<String> {
  ...
  private File userFileName;
    ...
    */
    public JiniFile(String FileName) {
        clear();
        this.userFileName = new File(FileName);
  ...
  public ArrayList<String> ReadSections() {
    ...
    for (int i=0; i < size(); i++) {
      ...
      if (s.startsWith("[") && s.endsWith("]")) {
          boolean add = list.add(s.substring(1,(s.length()-1)));
          System.out.println("ArrayLislistReadSections() list.add: "+add);
      }
    }
    return list;
  }
  ...
  /**
   * Call ReadDouble to read a string value from an INI file. Section identifies the
   * section in the file that contains the desired key. key is the name of the key
   * from which to retrieve the value. defaultValue is the double value to return if the:<br>
   * - Section does not exist.<br>
   * - Key does not exist.<br>
   * - Data value for the key is not assigned.<br>
   *
  ...
  public double ReadDouble(String Section, String key, double defaultValue) {
    ...
    if (ValuePosition(Section, key) > 0) {
      int strLen = key.length()+1;
      value = Double.valueOf(get(ValuePosition(Section, key)).toString().substring(strLen, get(ValuePosition(Section, key)).toString().length()));
    }
    return value;
  }
  ...
  /**
   * SectionPosition is used internally to determine the position of a section within the INI file
   * specified in FileName.
   * <p>
   * Section is the INI file section SectionPosition determines the position of.
   * <p>
   * SectionPosition returns an Integer value that indicates the position of the section in question.
   *
   * @param    Section             the section name
   *
  ...
  /**
   * Call WriteDouble to write a double value to an INI file. Section identifies the section in the
   * file that contain the key to which to write. key is the name of the key for which to set a value.
   * value is the double value to write.
   * <p>
   * <b>Note:</b> Attempting to write a data value to a nonexistent section or attempting to write data to a nonexistent
   *              key are not errors. In these cases, WriteDouble creates the section and key and sets its initial value
   *              to value.
   *
   * @param    Section             the section name
   * @param    key                 the key name
  ...
  public void WriteDouble(String Section, String key, double value) {
    String s = key+"="+Double.toString(value);
    this.addToList(Section, key, s);
  }
