...
*/
        ...
        else
        {
                ...
                VERIFY(m_pGetClipType= (PGetClipType) GetProcAddress(m_Tango,"LSX_GetClipType"));
                if (!m_pGetClipType) m_DLLResultat=-2;
...
int CTango::GetClipType (int *pClipType)
{
        if (m_pGetClipType)
        {
                return m_pGetClipType (m_LSID, pClipType);
        }
        else
                return -1;
}
