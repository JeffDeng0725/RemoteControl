...
//    var savePath: String = "" // 新添加的变量来存储路径07/22/2024
    ...
    // 重载方法：没有指定保存路径
    func startDownload(){
        startDownload(savePath: nil) // 无路径时传递nil或默认路径
    }

    // 重载方法：指定保存路径
    func startDownload(savePath: String?){
        if let path = savePath {
            _statusLabel.stringValue = "File Downloading to \(path) ..."
        } else {
