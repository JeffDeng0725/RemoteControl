...
class MainWindow: NSWindowController, NSApplicationDelegate, NSWindowDelegate  {
    ...
    @IBAction func runBrowseForFolder(_ sender: Any) {browseForFolder()}
    @IBOutlet weak var startDownloadButton: NSButton!
    @IBAction func runStartDownload(_ sender: Any) {startDownload()}
    ...
    init(controller: CameraController){
        ...
        startDownloadButton.isEnabled = false  // 初始化时禁用开始下载按钮
        ...
        browseButton.action = #selector(self.browseForFolder)
        startDownloadButton.action = #selector(self.startDownload)  // 设置开始下载按钮的动作

        self.savePathTextField.stringValue = NSSearchPathForDirectoriesInDomains(.picturesDirectory, .userDomainMask, true).first ?? ""
    ...
    @objc func startDownload(){
        if !savePathTextField.stringValue.isEmpty {
            ...
            downloadCtrl.startDownload(savePath: savePathTextField.stringValue)
    ...
    @objc func fileDownloadTapped(){
        ...
        let anApplication = NSApplication.shared
        _controller.getCameraModel().setSelectedVolume(0)
        downloadCtrl.startDownload(savePath: savePathTextField.stringValue) // 修改这一行，传递路径
        anApplication.runModal(for: downloadCtrl.window!)
    }
...
//            self.startDownloadButton.isEnabled = true
        })
    }
